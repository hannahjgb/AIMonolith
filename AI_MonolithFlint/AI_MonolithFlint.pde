...
void setup(){
  StatusListener listener = new StatusListener(){
    ...
    config.setOAuthConsumerKey("jecl0tnYv69qQmq3wsPHxboaR");
    config.setOAuthConsumerSecret("TaRTXLphduTEYwyMefDt1D45Wjhp6KqF7dI3LobEXACvixDvy7");
    config.setOAuthAccessToken("2734703166-p6ji4EJ5bykioknn1lz0XZZ05IoQJ9FXI6CwQ3r");
    config.setOAuthAccessTokenSecret("9lSNnpsf6AsiVxqHpr4QxygkHADBlChVTQwtVnjNC80Rx");
    ...
    query.locations(box);
    String[] tweetList = {"flint"}; 
    query.track(tweetList);
    twitterStream.filter(query);
}
//Albith's getTweets Function
    //List<Status> getTweets(String mySearch)
{ 
 // //  List<Status> filterQuery = new ArrayList<Status>(); 
    //try 
    //{
    ...
    //flintTweet1 = getTweets("#flintwatercrisis");
    ...
    //We'll need to populate these strings with RiTa outputs... 
...
String returnImage(List<Status> tweetList, String tweetList1, String character){
    if(tweetList.size() > 0) {
      println(character);
    tweetList1 = tweetList.get(1).getText();
    println(tweetList1);
